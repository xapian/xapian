...
(read-only, "replica") databases which these modifications propagate to.  For
...
following characteristics:
 ...
 - Searching (on the replica databases) and indexing (on the master database)
   can continue during synchronisation.

 - Data cached (in memory) on the replica databases is not discarded (unless
   it's actually out of date) as updates arrive, to ensure that searches
   continue to be performed quickly during and after updates.

 - Synchronising each replica database involves low overhead (both IO and CPU)
   on the server holding the master database, so that many replicas can be
   updated from a single master without overloading it.
...
of the above properties.  It is not intended to support merging modifications
made to separate copies of a database - there must only be a single master
database to which modifications are made.
...
Xapian 1.4.x).  It can cleanly handle the master switching database type (a
full copy is sent in this situation).

It doesn't make a lot of sense to support replication for the remote backend.
...
`Database::reopen()` is usually an efficient way to ensure that a
database is up-to-date with the latest changes.  Unfortunately, it does not
currently work as you might expect with databases which are being updated by
the replication client.  The workaround is simple; don't use the
...
which the "single master, multiple replicas" setup could be implemented.

 - Copy database from master to all replicas after each update, then swap the
   new database for the old.

 - Synchronise databases from the master to the replicas using rsync (or a
   similar tool).
 ...
 - Keep copy of database on master from before each update, and use a binary
   ...
   same changes to the databases on each replica.

 - Serve database from master to replicas over NFS (or other remote file
   system).

 - Use the "remote database backend" facility of Xapian to allow replica
   servers to search the database directly on the master.
...
Databases could be pushed to the replicas after each update simply by copying
the entire database from the master (using scp, ftp, http or one of the many
other transfer options).  After the copy is completed, the new database would
be made live by indirecting access through a stub database and switching what
it points to.
...
master server if a large number of replicas tried to synchronise with it.

Also, rsync will not reliably update the database in a manner which allows the
database on a replica to be searched while being updated - therefore, a copy or
...
transferred to the replicas, and then applied - avoiding the need for specific
calculations to be performed for each replica.
...
to be taken on the replica, since a patch from xdelta couldn't safely be
applied to a live database.

Serve database from master to replicas over NFS
-----------------------------------------------
...
master, and use that same machine as an NFS server.  Then, multiple replicas
can connect to that NFS server for searching the database. This setup is quite
...
the replicas will have to fetch all the blocks required for their search across
the network, which will likely result in extremely slow search times until the
cache on the replicas gets populated properly again.
...
of the work was performed on the master, while replicas remain largely idle.
