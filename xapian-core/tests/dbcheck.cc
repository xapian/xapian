...
postlist_to_string(const Xapian::Database & db, const string & tname)
{
	...
	result += "(" + om_tostring(*p) +
		", doclen=" + om_tostring(p.get_doclength()) +
		", wdf=" + om_tostring(p.get_wdf()) +
		posrepr + ")";
	need_comma = true;
    }
    return result;
...
docterms_to_string(const Xapian::Database & db, Xapian::docid did)
{
	...
	if (need_comma)
	    result += ", ";
	result += "Term(" + *t + ", wdf=" + om_tostring(t.get_wdf()) + posrepr + ")";
	need_comma = true;
    }
    return result;
}
...
termstats_to_string(const Xapian::Database & db, const string & term)
{
    ...
    result += ",cf=" + om_tostring(db.get_collection_freq(term));
...
dbstats_to_string(const Xapian::Database & db)
{
    ...
    result += ",al=" + om_tostring(db.get_avlength());
    result += ",ld=" + om_tostring(db.get_lastdocid());
	...
	Xapian::docid expected_lastdocid)
{
	...
	for (t = doc.termlist_begin(), t2 = db.termlist_begin(did);
	    ...
	    string posting_repr = "(" + om_tostring(did) + "," +
		    om_tostring(t.get_wdf()) + "/" + om_tostring(doclen) +
		    posrepr + ")";
	...
	for (Xapian::PostingIterator p = db.postlist_begin(*t);
	    ...
	    posting_repr += "(" + om_tostring(*p) + "," +
		    om_tostring(p.get_wdf()) + "/" + om_tostring(p.get_doclength()) +
		    posrepr + ")";
	    need_comma = true;
	}
