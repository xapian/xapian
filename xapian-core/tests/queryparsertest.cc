 ...
 *
 * Copyright (C) 2002,2003,2004,2005,2006,2007,2008,2010 Olly Betts
 *
...
static test test_or_queries[] = {
    ...
    // In 1.0.12, NON_SPACING_MARK was added as a phrase generator.
    { "\xd8\xa7\xd9\x84\xd8\xb1\xd9\x91\xd8\xad\xd9\x85\xd9\x86", "(\xd8\xa7\xd9\x84\xd8\xb1:(pos=1) PHRASE 2 \xd8\xad\xd9\x85\xd9\x86:(pos=2))" },
    // In 1.0.18, ENCLOSING_MARK and COMBINING_SPACING_MARK were added as
    // phrase generators.
    { "",
      "((\xe1\x80\x9d:(pos=1) PHRASE 5 \xe1\x80\x80:(pos=2) PHRASE 5 \xe1\x80\x95:(pos=3) PHRASE 5 \xe1\x80\x9e:(pos=4) PHRASE 5 \xe1\x80\x85:(pos=5)) OR (\xe1\x80\x9e:(pos=6) PHRASE 2 \xe1\x80\x99:(pos=7)) OR Z\xe1\x80\x80:(pos=8))" },
...
static bool test_qp_flag_wildcard1()
{
    ...
    TEST_STRINGS_EQUAL(qobj.get_description(), "Xapian::Query((abc:(pos=1) AND (muscle:(pos=2) OR musclebound:(pos=2)) AND main:(pos=3)))");
    return true;
#endif
}
...
static test test_stopword_group_or_queries[] = {
    ...
    { "test*", "(test:(pos=1) OR testable:(pos=1) OR tester:(pos=1))" },
    { "a test*", "(test:(pos=2) OR testable:(pos=2) OR tester:(pos=2))" },
    { "is a test*", "(test:(pos=3) OR testable:(pos=3) OR tester:(pos=3))" },
    { "this is a test*", "(test:(pos=4) OR testable:(pos=4) OR tester:(pos=4))" },
    { "this is a us* test*", "(user:(pos=4) OR test:(pos=5) OR testable:(pos=5) OR tester:(pos=5))" },
    { "this is a user test*", "(user:(pos=4) OR test:(pos=5) OR testable:(pos=5) OR tester:(pos=5))" },
    { NULL, NULL }
};
...
static test test_stopword_group_and_queries[] = {
    ...
    { "this is a us* test*", "(user:(pos=4) AND (test:(pos=5) OR testable:(pos=5) OR tester:(pos=5)))" },
    { "this is a user test*", "(user:(pos=4) AND (test:(pos=5) OR testable:(pos=5) OR tester:(pos=5)))" },
    { NULL, NULL }
};
