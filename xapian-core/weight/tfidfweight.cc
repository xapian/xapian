			 ...
			 Xapian::termcount, Xapian::termcount wdfdocmax) const
{
    ...
    double wt = get_wdfn(wdf, normalizations[0], wdfdocmax) *
...
double
TfIdfWeight::get_wdfn(Xapian::termcount wdf, char c, Xapian::termcount wdfdocmax) const
{
    switch (c) {
	case 'b':
	    if (wdf == 0) return 0;
