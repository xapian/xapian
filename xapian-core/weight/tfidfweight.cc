...
void
TfIdfWeight::init(double factor)
{
    factor_ = factor;
}
...
TfIdfWeight::get_idfn(Xapian::doccount termfreq, char c) const
{
	...
	case 'p':
	    ...
	    return log(double((N - termfreq) / termfreq));
	default:
	    AssertEq(c, 't');
	    return (log(double((N / termfreq))));
    }
}
