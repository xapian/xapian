...
static void measure(struct spanish_stemmer * z)
{   int j = 0;
    int k = z->k;
    char * p = z->p;
    z->posV = k; /* default */
...
static int look_for(struct spanish_stemmer * z, const char * s)
{   char * p = z->p;
    int length = strlen(s);
...
static int ends(struct spanish_stemmer * z, const char * s)
{  z->j = z->k - 1;
   return look_for(z, s);
}
...
static void setto(struct spanish_stemmer * z, const char * s)
{   int length = strlen(s);
...
/*
static int attachV(struct spanish_stemmer * z, const char * s)
...
static int after_posV(struct spanish_stemmer * z)
{   if (z->j < z->posV) return false;
    z->k = z->j+1;
    return true;
}

static int chopV(struct spanish_stemmer * z, const char * s) { return ends(z, s) && after_posV(z); }

static int chopV_try_u(struct spanish_stemmer * z, const char * s)
{   if (!ends(z, s)) return false;
    if (z->p[z->j-1] == 'g') look_for(z, "u");
    return after_posV(z);
}

static int attach2(struct spanish_stemmer * z, const char * s)
{   if (z->j < z->pos2) return false;
    setto(z, s);
    return true;
}

static int after_pos2(struct spanish_stemmer * z)
{   if (z->j < z->pos2) return false;
    z->k = z->j+1;
    return true;
}

static int chop2(struct spanish_stemmer * z, const char * s) { return ends(z, s) && after_pos2(z); }
...
static int attached_pronoun(struct spanish_stemmer * z)
{   char * p = z->p;
    int b = z->k;
    switch (p[b-2])
    {   case 'a':
...
static int verb_ending(struct spanish_stemmer * z)
{   switch (z->p[z->k-2])
    {   case 'a':
...
static int remove_suffix(struct spanish_stemmer * z)
{   char * p = z->p;
    int ess = p[z->k-1] == 's';
    if (ess) z->k--;
    switch (p[z->k-2])
...
static void tidy_up(struct spanish_stemmer * z)
{   switch (z->p[z->k-1])
    {   case 's':
...
static const char * irregular_forms[] = {
  ...
  ,
    ...
    "era/eras/Eramos/erais/eran/fui/fuiste/fue/fuimos/fuisteis/"
