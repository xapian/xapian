/* stem_porter.c: Porter stemming algorithm.
 *
 * ----START-LICENCE----
...
/* The Porter stemming algorithm is essentially the Porter stemming
...
/* The main part of the stemming algorithm starts here. z->p is a buffer
   ...
   should be done before porter_stem(...) is called.
...
static int cons(struct porter_stemmer * z, int i)
{   switch (z->p[i])
...
static int m(struct porter_stemmer * z)
{   int n = 0;
    int i = 0;
    while(true)
    {   if (i > z->j) return n;
...
static int vowelinstem(struct porter_stemmer * z)
{   int i;
...
static int doublec(struct porter_stemmer * z, int i)
{   if (i < 1) return false;
...
static int cvc(struct porter_stemmer * z, int i)
{
...
static int ends(struct porter_stemmer * z, const char * s, int length)
{
...
static void setto(struct porter_stemmer * z, const char * s, int length)
{
...
static void r(struct porter_stemmer * z, const char * s, int length)
{
...
static void step_1ab(struct porter_stemmer * z)
{   if (z->p[z->k] == 's')
...
static void step_1c(struct porter_stemmer * z)
{
    /*    if (ends(z, "y", 1) && z->j > 0 && cons(z, z->k - 1)) z->p[z->k] = 'i'; */

    /*  porter version ? */
    if (ends (z, "y", 1) && vowelinstem (z)) z->p[z->k] = 'i';
}
...
static void step_2(struct porter_stemmer * z)
{   switch (z->p[z->k - 1])
    {
        case 'a':
...
static void step_3(struct porter_stemmer * z)
{   switch (z->p[z->k])
    {
        case 'e':
...
static void step_4(struct porter_stemmer * z)
{   switch (z->p[z->k - 1])
    {   case 'a':
...
static void step_5(struct porter_stemmer * z)
{   z->j = z->k;
    if (z->p[z->k] == 'e')
    {   int a = m(z);
...
extern const char * porter_stem(void * z_, const char * q, int i0, int i1)
{
    struct porter_stemmer *z = (struct porter_stemmer *) z_;
    int p_size = z->p_size;
...
{
    struct porter_stemmer * z = (struct porter_stemmer *) malloc(sizeof(struct porter_stemmer));
    z->p = 0; z->p_size = 0;
    z->irregulars = create_pool(irregular_forms);
    return (void *)z;
}

extern void closedown_porter_stemmer(void * z_)
{
    struct porter_stemmer *z = (struct porter_stemmer *) z_;
