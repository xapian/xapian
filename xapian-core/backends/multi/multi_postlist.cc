...
MultiPostList::next(double w_min)
{
    if (docids_size == 0) {
       ...
       docids_size = j;
       make_heap(docids, docids + docids_size);
    } else {
       Xapian::docid old_did = docids[0];
       pop_heap(docids, docids + docids_size);
       ...
       if (pl->at_end()) {
           ...
           docids[docids_size - 1] = pl->get_docid();
           push_heap(docids, docids + docids_size);
       }
    }
