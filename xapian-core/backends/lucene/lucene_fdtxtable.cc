...
{
    //File name has fixed suffix '.fdx'
...
{
    //File name has fixed suffix '.fdt'
...
{
    LOGCALL(DB, bool, "LuceneFdtxTable::open", NO_ARGS);
...
LuceneFdtxTable::get_record(Xapian::docid did, map<int, string> & string_map,
    ...
    LOGCALL(API, string, "(not realized)LuceneFdtxTable::get_record", did);
    ...
    for (int i = 0; i < field_count; ++i) {
        ...
        LOGLINE(DB, "LuceneFdtxTable::get_record, field_num=" << field_num <<
                    ", bits=" << (int)bits << ", record=" << record);
