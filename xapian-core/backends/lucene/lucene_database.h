...
class LuceneDatabase : public Xapian::Database::Internal {
    ...
    /** File stat.xapian, statistic data for Xapian.
     */
  ...
  public:
    ...
    /** Return doc count in a particular segment
     */
    ...
    /** No used, lucene3.6.2 has no total_lenght
     */
    totlen_t get_total_length() const;

    /** No used, Lucene3.6.2 has no total_length, so avlength lacks
     */
     ...
     *  There's no field number parameter, I think it should stored
     *  in Query Object first, and pass it to somewhere in DB.
     */
    Xapian::doccount get_termfreq(const string &tname) const;

    /** No used, lucene3.6.2 has no collection_freq
     */
    ...
    /** No used, lucene3.6.2 has no value_lower_bound
     */
    string get_value_lower_bound(Xapian::valueno slot) const;

    /** No used, lucene3.6.2 has no value_upper_bound
     */
    string get_value_upper_bound(Xapian::valueno slot) const;
    //Xapian::termcount get_doclength_lower_bound() const;
    //Xapian::termcount get_doclength_upper_bound() const;

    /** wdf_upper_bound is caculated by lucece-copydatabase.cc, and stores it
     *  in file stat.xapian
     */
