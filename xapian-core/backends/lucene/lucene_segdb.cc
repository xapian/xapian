...
LuceneSegdb::create_and_open_tables()
{
    ...
    //Changing function name to open() seems better
...
{
    LOGCALL(DB, Xapian::doccount, "LuceneSegdb::get_termfreq", lterm);
...
{
    LOGCALL(DB, LucenePostList *, "LuceneSegdb::open_post_list", lterm);

    LuceneTermInfo term_info;
    bool b = index_reader.seek(lterm, term_info);
    if (false == b) {
        LOGLINE(API, "LuceneDatabase::open_post_list index_reader.seek failed");
        RETURN(NULL);
    }

    LOGLINE(API, "LuceneDatabase::open_post_list index_reader.seek success");
...
{
    LOGCALL(DB, void, "LuceneSegdb::get_fieldinfo", field_info);
...
LuceneSegdb::get_luceneterm(const string & query, LuceneTerm & lterm) const
{
    ...
    //Query format must fit the foramt like: 'field_name:term'
    ...
    // No matching, query string format is wrong, or no related field, abort()
    if (-1 == fn) {
