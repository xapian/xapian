...
{   int key_len = GETK(key, 0) - C2;
    int k_len = GETK(k, 0) - C2;
    int k_smaller = k_len < key_len ? k_len : key_len;
    int i; for (i = K1; i < k_smaller; i++)
    {   int diff = (int) key[i] - k[i];
        if (diff != 0) return diff;
    }
    if (key_len != k_len) return key_len - k_len;
    return GETC(key, key_len) - GETC(k, k_len);
}
...
static void enter_key(struct Btree * B, struct Cursor * C, int j, byte * kq, byte * kp)
{
    ...
    {   /*  byte * p = C[j - 1].p;  -- see below */
        ...
        {   int4 n = C[j - 1].n;
            ...
            int kq_len = GETK(kq, 0) - C2;
            int kp_len = GETK(kp, 0) - C2;
            int ck = GETC(kp, kp_len);
            int i;

            if (j > 1) i = kp_len; else
            {   i = K1;
...
#define B_SEQUENTIAL    32  /* 1 byte - boolean */
...
static struct Btree * basic_open(const char * name, int revision_supplied, uint4 revision)
{
    ...
    {   byte * baseA = read_base(s, name_len, 'A');
        ...
        B->faked_root_block =GETINT1(base, B_HAVE_FAKEROOT);
        B->sequential =      GETINT1(base, B_SEQUENTIAL);
                             /*B->faked_root_block;*/
...
extern int Btree_close(struct Btree * B, uint4 revision)
{
    ...
    SETINT1(B->base, B_HAVE_FAKEROOT, B->faked_root_block);
    SETINT1(B->base, B_SEQUENTIAL, B->sequential);
...
static int prev_for_sequential(struct Btree * B, struct Cursor * C, int dummy);
static int next_for_sequential(struct Btree * B, struct Cursor * C, int dummy);
...
static struct Btree * open_to_read(const char * name, int revision_supplied, uint4 revision)
{
    ...
    B->prev = B->sequential ? prev_for_sequential : prev;
    B->next = B->sequential ? next_for_sequential : next;
...
extern int Btree_create(const char * name, int block_size)
{
    ...
    {   error = BTREE_ERROR_BITMAP_CREATE;
        ...
        SETINT1(b, B_HAVE_FAKEROOT, true);
        SETINT1(b, B_SEQUENTIAL, true);
        {   int h = sys_open_to_write(s);
            if ( ! (valid_handle(h) &&
...
static void print_key(byte * p, int c, int j) /* j unused */
{   byte * k = key_of(p, c);
    int l = GETK(k, 0);
...
{   struct Btree * B = Btree_open_to_write(name);
    ...
    if (opts & 8)
    {
        printf("base%c  Revision *%ld  levels %d  root [%ld]",
                B->base_letter,
                B->revision_number,
                B->level,
                C[B->level].n);
        if (B->faked_root_block) printf("(faked)");
        printf("  blocksize %d  items %ld  lastblock %ld\n",
                B->block_size,
                B->item_count,
                B->last_block);
    }
