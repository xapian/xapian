...
void checkterm(struct DA_file * p, struct DA_term_info * t, int i)
{     if (t->shsize == 0) return;
      ...
      {  struct DA_postings * q = DA_open_postings(t,p);
         int i; for(i=1;;i++)
         {  DA_read_postings(q,1,34000);      /* or (q,0,0) for expanded ranges */
...
{  int x = FLIMSY;
   ...
   {  struct DA_file * p;
      ...
      makekform("compani",b); DA_term(b,&t,p); checkterm(p,&t,1);
      makekform("new",b);     DA_term(b,&t,p); checkterm(p,&t,2);
      makekform("year",b);    DA_term(b,&t,p); checkterm(p,&t,3);
