...
void
BtreeCheck::block_check(Cursor_ * C_, int j, int opts)
{
    byte * p = C_[j].p;
    uint4 n = C_[j].n;
    size_t c;
	...
	if (c > significant_c && Item_(p, c - D2).key() >= item.key())
	    failure(50);
    }
	...
	if (j == 1 && c > DIR_START)
	    if (Item_(q, DIR_START).key() < Item_(p, c).key())
		failure(70);
	...
	if (j > 1 && c > DIR_START && DIR_END(q) > DIR_START + D2 &&
	    Item_(q, DIR_START + D2).key() < Item_(p, c).key())
	    failure(80);
...
void
BtreeCheck::check(const char * tablename, const string & path,
...
void BtreeCheck::report_cursor(int N, const Cursor_ * C_) const
{
    out << N << ")\n";
