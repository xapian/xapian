...
#ifndef XAPIAN_INCLUDED_BRASS_MODIFIEDPOSTLIST_H
#define XAPIAN_INCLUDED_BRASS_MODIFIEDPOSTLIST_H
...
class BrassModifiedPostList : public BrassPostList {
    /// Modifications to apply to the BrassPostList.
    //@{
  ...
  public:
    /// Constructor.
    BrassModifiedPostList(Xapian::Internal::RefCntPtr<const BrassDatabase> this_db_,
			  const string & term_,
			  const map<Xapian::docid, pair<char, Xapian::termcount> > & mods_)
	: BrassPostList(this_db_, term_, true),
	  mods(mods_), it(mods.begin()), poslist(0)
    { }

    ~BrassModifiedPostList();
...
#endif // XAPIAN_INCLUDED_BRASS_MODIFIEDPOSTLIST_H
