...
#define MAX_ENTRIES_OF_DOCLEN_CHUNK 2000

/* A chunk of skip list has at most so many entries. */
#define MAX_ENTRIES_OF_SKIPLIST_CHUNK 2000
...
/* Postlist (docid, wdf) is encoded using skip-list format.
 * Assuming we have a postlist (1,2), (3,5), (4,7), (8,12), 
 * document ids are encoded by delta, so actually we encode (0,2), (2,5), (1,7), (4,12).
 * For faster searching, some skip information was added to original array.
 * Each skip information segment begins with -1, and followed by p_offset and d_offset.
 * Assuming p is a pointer (const char*) to a stinrg chunk, 
 * if we add p_offset to p, then we can get document id cur_did+d_offset,
 * cur_did represents current document id when we are at position p.
 * So the postlist list is to be encoded as 
 * (-1,2,2,0,2,2,5,-1,2,5,1,7,4,12).
 * For example, (-1,2,5) is a skip information segment, -1 is an indicator.
 * 2 is p_offset and 5 is d_offset.
 * After finishing reading this segment, p points to 1, and current document id is 1+0+2=3
 * when p_offset is added to p, p will point to 4, and we can get document id 3+5=8.
 * We assume each document id and wdf can be encoded within 8 bits (a char) for convenience in the above,
 * but actually a number may need more than 8 bits, 
 * so extra calculation is needed to get correct offset. */


/// this class is designed to encode a postlist (map<docid,wdf>) using skip-list format
class SkipList {
    
    /// a vector to save postlist and skip-info segment
	vector<unsigned> src;
    
    /// get deltas of document ids
	void genDiffVector();
    
    /// how many bytes to encode a number @n?
    ...
    /// the start and end of the postlist
...
/// this class is designed to read the chunk generated by class SkipList
class SkipListReader {
    
    /// pointer to the start of the skip-list chunk
    const char* ori_pos;
    
    /// pointer to current position
	const char* pos;
    
    /// pointer to the end of the skip-list chunk
	const char* end;
    
    /// whether we have run off the chunk
    bool at_end;
    
    /// current document id
    Xapian::docid did;
    
    /// first document id in this chunk
...
/// this class is designed to merge changes of a postlist
class SkipListWriter {
    
    /// original chunk
    string& chunk_from;
    
    /// first document id in original chunk
    Xapian::docid first_did;
    
    /// if original chunk is the first chunk
    bool is_first_chunk;
    
    ///postlist table, used to insert new chunk
    BrassPostListTable* postlist_table;
    
    /// if original chunk is the last chunk
    bool is_last_chunk;
    
    /// merge old map and new map, the @new_postlist is valid.
    bool get_new_postlist();
    
    /// store new postlist
    map<Xapian::docid,Xapian::termcount> new_postlist;
    
    /// the start and the end of the changes (map<docid,wdf>)
    map<Xapian::docid,Xapian::termcount>::const_iterator changes_start, changes_end;
    
    /// the number of entries for the term
    Xapian::termcount num_of_entries;
    
    /// the collection frequency for the term
  ...
  public:
    ...
    /// merge changes for the postlist of term @term
...
class BrassPostList : public LeafPostList {
    ...
    /// We have run off the list only when we have run off current chunk and current chunk is the last chunk
