...
void
BrassTable_base::extend_bit_map()
{
...
uint4
BrassTable_base::next_free_block()
{
    uint4 i;
    int x;
    for (i = bit_map_low;; i++) {
...
bool
BrassTable_base::find_changed_block(uint4 * n)
{
...
bool
BrassTable_base::block_free_now(uint4 n)
{
    uint4 i = n / CHAR_BIT;
    int bit = 0x1 << n % CHAR_BIT;
