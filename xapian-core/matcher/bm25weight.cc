...
BM25Weight::calc_termweight() const
{
    ...
    DEBUGMSG(WTCALC, "Statistics: N=" << stats.collection_size <<
	     " n_t=" << termfreq);
	...
	DEBUGMSG(WTCALC, " R=" << stats.rset_size << " r_t=" << rtermfreq);
	...
	// (stats.collection_size - stats.rset_size + rtermfreq) since the
	// number of relevant documents not indexed by this term can't be more
	// than the number of documents not indexed by this term.

	Assert(termfreq >= rtermfreq);
	Assert(termfreq <= stats.collection_size - stats.rset_size + rtermfreq);

	tw = ((rtermfreq + 0.5) *
	      (stats.collection_size - stats.rset_size - termfreq + rtermfreq + 0.5)) /
	     ((stats.rset_size - rtermfreq + 0.5) *
