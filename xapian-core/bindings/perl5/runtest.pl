...
$baz = OmQuery->new_OmQueryList($omsee::OM_MOP_OR,
			    [ OmQuery->new_OmQueryTerm("giraff"),
			      OmQuery->new_OmQueryTerm("lion") ]);
print $baz->get_description() . "\n";

$myquery = OmQuery->new_OmQueryList($omsee::OM_MOP_OR,
               [ OmQuery->new_OmQueryList($omsee::OM_MOP_AND,
	            [ OmQuery->new_OmQueryTerm("one", 1, 1),
		      OmQuery->new_OmQueryTerm("three", 1, 3) ]),
	         OmQuery->new_OmQueryList($omsee::OM_MOP_OR,
