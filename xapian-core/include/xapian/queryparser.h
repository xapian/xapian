...
class XAPIAN_VISIBILITY_DEFAULT QueryParser {
  public:
    ...
    typedef enum {
	/** Use terms generated by the prefix to generate inline query terms.
	 *
	 *  Most query terms will be generated in this manner.
	 */
	PREFIX_INLINE,

	/** Use terms generated by the prefix to generate filter terms.
	 *
	 *  This should be set for terms which are used to filter the set of
	 *  matching documents.  The terms will be "pulled out" of the query,
	 *  such that their location in the query, and any surrounding
	 *  operators, will not have an effect.  A "filter" query will then be
	 *  constructed from all the filter terms, and used to restrict the
	 *  documents which match the query, without affecting their rankings.
	 *
	 *  The filter query will be constructed by combining all filter terms
	 *  with a given prefix (or list of prefixes) together with an OR
	 *  operator.  Then, if there are filters with different prefixes, the
	 *  resulting groups will be combined with the AND operator.
	 */
	PREFIX_FILTER
    } prefix_type;
     ...
     *  For example:
     *
     *    qp.add_prefix("author", "A", Query::PREFIX_INLINE);
     *
     ...
     *  main difference between these old methods and this method is that this
     *  method allows multiple prefixes to be set for a single field: even if
     *  the old methods are called repeatedly with the same value for the
     *  "field" parameter, only the first call will have any effect.
     *
     *  @param field   The user visible field name
     *  @param prefix  The term prefix to map this to
