...
class OmTermListIteratorMap : private OmTermListIterator {
    private:
	friend class OmDocument; // So OmDocument can construct us
	...
	OmTermListIteratorMap(Internal *internal_)
	    : OmTermListIterator(NULL), internal(internal_) { }
	 ...
	 *  copying is also cheap.
	 */
	OmTermListIteratorMap(const OmTermListIteratorMap &other);

        /** Assignment is allowed.  The internals are reference counted,
	 *  so assignment is also cheap.
	 */
	void operator=(const OmTermListIteratorMap &other);
