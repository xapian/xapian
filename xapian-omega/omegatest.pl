...
print_to_file $test_template, '$slice{$split{: ,$log{$cgi{LOG},$log{' . $test_log . '}}},0}';
testcase('filename can\'t contain ".."', 'LOG=evil/parent/..');
if (-f $test_log) {
  remove_tree($test_log);
  print "log entry not lazily evaluated for bad log file name\n";
  ++$failed;
}
testcase('open failed', 'LOG=open/should/fail', 'ENTRY=$log{' . $test_log . '}');
if (-f $test_log) {
  remove_tree($test_log);
  print "log entry not lazily evaluated for open failure\n";
  ++$failed;
}
...
testcase('CHexample.net93org5', 'B=Hexample.org', 'B=Hexample.net');
testcase('!AGmisc.test5', 'N=Gmisc.test');
testcase('CHexample.net93org!AGmisc.misc64test5', 'B=Hexample.org', 'B=Hexample.net', 'N=Gmisc.test', 'N=Gmisc.misc');
testcase('$1!200406125', 'DATEVALUE=1', 'START=20040612');
testcase('$1!200406125', 'START.1=20040612');
testcase('.2$1!200406125', 'DATEVALUE=1', 'START=20040612', 'COLLAPSE=2');
testcase('.2$1!200406125', 'START.1=20040612', 'COLLAPSE=2');
testcase('.2$!20040612~305', 'START=20040612', 'SPAN=30', 'COLLAPSE=2');
testcase('$!20040612.201604125', 'START=20040612', 'END=20160412');
testcase('.25', 'COLLAPSE=2');
testcase('5');
testcase('15', 'SORT=1');
testcase('14', 'SORT=+1');
testcase('15', 'SORT=-1');
testcase('1-2+3-R5', 'SORT=+1-2+03,-27');
testcase('5', 'SORTREVERSE=1');
testcase('14', 'SORT=1', 'SORTREVERSE=1');
testcase('15', 'SORT=+1', 'SORTREVERSE=1');
testcase('14', 'SORT=-1', 'SORTREVERSE=1');
testcase('1-2+3-R4', 'SORT=+1-2+03,-27', 'SORTREVERSE=1');
testcase('5', 'SORTAFTER=1');
testcase('17', 'SORT=1', 'SORTAFTER=1');
testcase('16', 'SORT=+1', 'SORTAFTER=1');
testcase('17', 'SORT=-1', 'SORTAFTER=1');
testcase('1-2+3-R7', 'SORT=+1-2+03,-27', 'SORTAFTER=1');
testcase('5', 'SORTREVERSE=1', 'SORTAFTER=1');
testcase('16', 'SORT=1', 'SORTREVERSE=1', 'SORTAFTER=1');
testcase('17', 'SORT=+1', 'SORTREVERSE=1', 'SORTAFTER=1');
testcase('16', 'SORT=-1', 'SORTREVERSE=1', 'SORTAFTER=1');
testcase('1-2+3-R6', 'SORT=+1-2+03,-27', 'SORTREVERSE=1', 'SORTAFTER=1');
testcase('5', 'DOCIDORDER=A');
testcase('1', 'DOCIDORDER=D');
testcase('9', 'DOCIDORDER=X');
testcase('9', 'DOCIDORDER=x');

# Test old filter encoding.
print_to_file $test_template, '$filters{x}';
testcase('Hexample.net~Hexample.org~.~~', 'B=Hexample.org', 'B=Hexample.net');
testcase('!Gmisc.test~.~~', 'N=Gmisc.test');
testcase('Hexample.net~Hexample.org~!Gmisc.misc~!Gmisc.test~.~~', 'B=Hexample.org', 'B=Hexample.net', 'N=Gmisc.test', 'N=Gmisc.misc');
...
print_to_file $test_template, '$hash{$cgi{text},md5}';
...
print_to_file $test_template, '$or{$list{$map{$range{1,100},$set{num,$random{$cgi{x}}}$or{$lt{$opt{num},0},$gt{$opt{num},$cgi{x}}}},},OK}';
testcase('OK', 'x=6');

# Regression test - original implementation truncated data at first zero byte.
print_to_file $test_template, '$hash{$cgi{text}$chr{0},md5}';
testcase('e2a3e68d23ce348b8f68b3079de3d4c9', "text=test");

print_to_file $test_template, '$hash{$cgi{text},invalidhash}';
...
test_scriptindex 'UNHTML and UNXML actions',
't=<b>No</b>table

t=<p>foo</p>d<p>bar<B>b</B></p>
';
print_to_file $test_template, '$list{$map{$split{$cgi{DOCIDS}},$field{h,$_}:$field{x,$_}},|}';
testcase('Notable:No table|foo d barb:foo d bar b', 'DOCIDS=1 2');
...
print_to_file $test_indexscript, "id : boolean=Q unique=Q\nid f : field\n";
test_scriptindex_error 'missing unique field',
  "<stdin>:4: error: UNIQUE action unused in this record",
  "id=1\nf=wan\n\nf=";
