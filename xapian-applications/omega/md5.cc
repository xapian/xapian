 ...
 *
 * + Use WORDS_BIGENDIAN (probed by autoconf) to check endianness.
 * + uint32 -> uint32_t (as defined by netinet/in.h).
 * + Internal functions are now static.
 * + Fix sizeof(ctx) to sizeof(*ctx) (bug picked up by Stanford checker on
 *   this code incorporated into the Linux kernel).
