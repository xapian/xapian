...
/**
 * Opens the database storing fileId,revision->{lines}.
 *
 ...
 **/
int
cvs_line_db::do_open(const string & filename, bool read_only)
{
...
/**
 * Gets a set of lines given a fileId and revision.
 *
 * @param fileId the recno of the filename to get.
 * @param revision the revision where the result lines are affected.
 * @param result the container to store the result set of lines.
 *
 * @return error code of the get operation.
 **/
int
cvs_line_db::get(unsigned int fileId, const string & revision, set<unsigned int> & result)
{
    result.clear();
...
/**
 * Puts a (fileId,revision)->line into the database.
 *
