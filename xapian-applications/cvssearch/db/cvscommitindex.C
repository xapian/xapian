...
// I think the reason is that with a set, Xapian ranks those commit higher than contain
...
static void write_OM_database( const string & database_dir,
                               const map<unsigned int, list<string> > & commit_code_words,
...
#if !DB_ONLY
    compare(commit_code_words, commit_comment_words, commit_code_words1, commit_comment_words1);
#endif
    fout.close();
...
void write_DB_database( const string & database_file,
                        const map<unsigned int, list<string> > & commit_all_words)
{
  // ----------------------------------------
  ...
  map<unsigned int, list<string> >::const_iterator t;
...
#warning "when is fileid zero?"
      ...
      if (db_file.get_commit(fileid, i->first, commitid) == 0)
	{
	  ...
	  if ( commit_code_words.find( commitid+offset ) == commit_code_words.end() ) {
	    list<string> empty;
	    commit_code_words[ commitid+offset ] = empty;
	  }
	  if ( commit_all_words.find( commitid+offset ) == commit_all_words.end() ) {
	    list<string> empty;
	    commit_all_words[ commitid+offset ] = empty;
	  }
	  ...
	  if (commit_comment_words[commitid+offset].empty())
		...
		for( list<string>::const_iterator i = words.begin(); i != words.end(); i++ ) {
		  if ( stopSet.find(*i) == stopSet.end() ) {
		    ...
		    commit_all_words[commitid+offset].push_back(*i);		  
		  }
		}
	      }
	    }
	  ...
	  for( list<string>::iterator s = symbol_terms.begin(); s != symbol_terms.end(); ++s ) {
	    if ( stopSet.find(*s) == stopSet.end() ) {
	      commit_code_words[commitid+offset].push_back(*s);
	      commit_all_words[commitid+offset].push_back(*s);
	    }
	  }
	}
    }
