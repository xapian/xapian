...
typedef std::pair<Xapian::docid, double> docid_to_score;
...
try {
    ...
    string query_file = "~/Downloads/INEX/query.txt";
    string qrel_file  = "~/Downloads/INEX/qrel.txt";
    int n_entries     = 100;

    ltr.create_ranker(Xapian::Letor::RANKER_SVM);
    ltr.set_training_file_svm(query_file, qrel_file, n_entries);
    ...
    set<docid_to_score, MyTestCompare> s;

    for (map<Xapian::docid, double>::iterator mset_it = letor_mset.begin();
        mset_it != letor_mset.end(); ++mset_it)
    {
       s.insert(*mset_it);
    }
