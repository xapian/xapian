# Get XAPIAN_CXXFLAGS, XAPIAN_LIBS, and XAPIAN_VERSION from xapianletor-config and
# AC_SUBST() them.
...
[
  ...
  xapian_config_to_check_for="ifelse([$3], [], xapianletor-config, [$3])"
  ...
  *)
    ...
    xapian_config_to_check_for=$XAPIANLETOR_CONFIG
    XAPIANLETOR_CONFIG=
    ;;
  esac
  AC_PATH_PROG(XAPIANLETOR_CONFIG, "$xapian_config_to_check_for")
  if test -z "$XAPIANLETOR_CONFIG"; then
    ifelse([$2], ,
      [ifelse([$1], , [
	dnl Simple check to see if the problem is likely to
	dnl be that we're using a "packaged" xapian-letor but
	dnl only have the runtime package installed.
	for sfx in '' 32 64 ; do
	  set /usr/lib$sfx/libxapianletor.so.*
	  if test "/usr/lib$sfx/libxapianletor.so.*" != "$1" ; then
	    if test -r /etc/debian_version ; then
	      pkg="libxapian-dev"
	    else
	      pkg="xapian-letor-devel"
	    fi
	    AC_MSG_ERROR([Can't find xapianletor-config, although the xapian-letor runtime library seems to be installed.  If you've installed xapian-letor from a package, you probably need to install an extra package called something like $pkg in order to be able to build code using the Xapian library.])
	  fi
	done
	AC_MSG_ERROR([Can't find xapianletor-config.  If you have installed the Xapian library, you need to add XAPIANLETOR_CONFIG=/path/to/xapianletor-config to your configure command.])],
	:)],
      [$2])
  else
...
dnl AM_PROG_LIBTOOL to enable it if one of these is expanded later.
    XAPIAN_VERSION=`$XAPIANLETOR_CONFIG --version|sed 's/.* //;s/_.*$//'`
    XAPIAN_CXXFLAGS=`$XAPIANLETOR_CONFIG --cxxflags`
    AC_PROVIDE_IFELSE([LT_INIT],
      [XAPIAN_LIBS=`$XAPIANLETOR_CONFIG --ltlibs`],
	...
	[AC_PROVIDE_IFELSE([AM_PROG_LIBTOOL],
	  ...
	  [XAPIAN_LIBS=`ac_top_srcdir="$ac_top_srcdir" $XAPIANLETOR_CONFIG --from-xo-lib-xapianletor --libs`
	  m4_ifdef([LT_INIT],
