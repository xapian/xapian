...
Feature::Internal::termfreq() const
{
    ...
    for (Xapian::TermIterator qt = feature_query.get_terms_begin(); qt != feature_query.get_terms_end(); ++qt) {
	docterms.skip_to(*qt);
...
Feature::Internal::collection_length() const
{
    ...
    if (!feature_db.get_metadata("collection_len_title").empty() &&
	!feature_db.get_metadata("collection_len_body").empty() &&
	!feature_db.get_metadata("collection_len_whole").empty()) {
	...
	temp_count += feature_db.get_collection_freq(*dt);
    }
    len["title"] = temp_count;
